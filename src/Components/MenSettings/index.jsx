...
export const MenSettings = () => {
  const { productActiveCategory, productActiveSubCategory } = new ManageCategoriesApi();
  const { createMenSettings, getMenSettings } = new SettingsAPI();
  const { collectionsActive, brandsActive } = new productCateoriesAPI();
  const { data: settings, refetch } = useQuery(["settings"], getMenSettings);
  ...
  const { mutate: createSettingsMutate } = useMutation(createMenSettings, {
    onSuccess: () => {
  ...
  return (
    <>
      <Breadcrumb currentPage={"Men Settings"} />
      <ProfileCard>
