...
export const WomenSettings = () => {
  const { productActiveCategory, productActiveSubCategory } = new ManageCategoriesApi();
  const { createWomenSettings, getWomenSettings } = new SettingsAPI();
  const { collectionsActive, brandsActive } = new productCateoriesAPI();
  const { data: settings, refetch } = useQuery(["settings"], getWomenSettings);
  ...
  const { mutate: createSettingsMutate } = useMutation(createWomenSettings, {
    onSuccess: () => {
  ...
  return (
    <>
      <Breadcrumb currentPage={"Women Settings"} />
      <ProfileCard>
