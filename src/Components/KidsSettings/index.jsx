...
export const KidsSettings = () => {
  const { productActiveCategory, productActiveSubCategory } = new ManageCategoriesApi();
  const { createKidsSettings, getKidsSettings } = new SettingsAPI();
  const { collectionsActive, brandsActive } = new productCateoriesAPI();
  const { data: settings, refetch } = useQuery(["settings"], getKidsSettings);
  ...
  const { mutate: createSettingsMutate } = useMutation(createKidsSettings, {
    onSuccess: () => {
  ...
  return (
    <>
      <Breadcrumb currentPage={"Kids Settings"} />
      <ProfileCard>
